...
class FullController extends Controller
{
    ...
    public function getSharedAvailability(Request $request, $id)
    {
        ...
        // ── Helper: total availability across all boats for one date ───
        $calcDate = function (array $boats, string $date) use ($boatIndex): array {
            ...
            foreach ($boats as $boat) {
                ...
                if ($rec === null) {
                    // No closeddate records → boat fully open
                    $totalAvailable += $capacity;
                } elseif ($rec['blocked']) {
                    // Blocked → contributes 0
                } else {
                    // Partial booking (type 1)
                    $avail = max(0, $capacity - $rec['qtty']);
                    $totalAvailable += $avail;
                    $totalBooked += $rec['qtty'];
                }
            }
