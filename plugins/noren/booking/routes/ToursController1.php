...
class ToursController1 extends Controller
{
...
public function getTourClosedDates($id)
{
     ...
     */
    public function getTours($members = 1, $typeId = null, $categoryId = null,  $date = null, $classId = null)
    {
    ...
    header('Access-Control-Allow-Headers: *');
        ...
        [$adults, $kids, $children, $typeId, $categoryId, $classId, $date ] = [
            $input['adults'],
            $input['kids'],
            $input['children'],
            $input['type'],
        ...
        $members = $adults + $kids;
        $allMembers = $adults + $kids + $children;

        $tours = $this->getToursQuery($typeId, $categoryId, $classId);
        if (!$tours->count()) {
	...
	{
	    $input = Input::only('adults', 'kids', 'children', 'type', 'category', 'classes' ,'date');
	
		//Добавляем ключи по умолчанию, чтобы не было Undefined array key
	    $defaults = [
	        'adults'   => null,
	        'kids'     => 0,
	        'children' => 0,
    ...
    // --------------------------------------------------
    private function getToursQuery($typeId, $categoryId, $classId)
    {
    ...
    private function formatTour($tour, $date, $members)
    {
        ...
        return [
            ...
            'limited_images' => array_slice($tour->images_with_thumbs, 0, 6),
            'badge' => $tour->badge ? [
    ...
    private function getTourStatus($tour, $date, $members, $available)
    {
        ...
        if ($members > $available or $members>$tour->capacity) {
            return 'exceeded';
        }
    ...
    private function sortAndGroupTours(array $rows)
    {
       ...
       // usort($our, $sortByStatus);
        ...
        usort($others, $sortByStatus);

        //return array_merge(array_values($types1), array_values($our), array_values($partner));
        return array_merge(array_values($types1), array_values($others));
    }
}
