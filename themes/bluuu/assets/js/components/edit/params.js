const ParamSelector={
    ...
    <div id="params" class="bottom-popup popup-small bg-white p-1 rounded-1 order-lg-0 order-1">
        ...
        <div v-show="loading==false" >
            <input ref="travel_dateInput" class="d-none">
    ...
    watch: {
        ...
        totalParticipants() {
            // обновляем список закрытых дат
            if (this.calendar) {
                this.calendar.set('disable', this.filteredClosedDates);
            }

            // проверяем, не стала ли выбранная дата недоступной
            if (this.selectedDate && this.filteredClosedDates.includes(this.selectedDate)) {
    ...
    computed: {
        ...
        filteredClosedDates() {
            if (!Array.isArray(this.closed)) return [];
            return this.closed
                .filter(item => this.totalParticipants > item.qtty)
                .map(item => item.date);
        },
        total() {
            return this.adults + this.kids + this.children;
    ...
    methods: {
        ...
        async checkDate() {
            this.loading = true;
            try {
                const response = await axios.get('/api/t/closed/'+this.tour.id)
        ...
        showCaledar() {
            ...
            this.calendar = flatpickr(this.$refs.travel_dateInput, {
                inline: true,
                monthSelectorType: 'static',
                dateFormat: "Y-m-d",
                disable: this.filteredClosedDates,
                defaultDate: this.order.travel_date ,
        ...
        save() {
            ...
            if(this.selectedDate!=null)  {
                this.order.travel_date = this.selectedDate;
            }
            this.$emit('close');
        },
