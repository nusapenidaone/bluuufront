const ExtrasSelector = {
    ...
    template: `
        <div class="p-4 border-black-10 bg50 rounded-1 mb-3 bg-white">
            <div class="d-flex fz-12 cursor" v-on:click="show=!show">
    ...
    methods: {
        showExtras(extras){
            ...
            availableQty(extras){
                const item = this.order.selectedExtras.find(e => e.id === extras.id);
        ...
        increaseQty(extra) {
            ...
            const item = this.order.selectedExtras.find(e => e.id === extra.id);
            const alreadyInCart = item ? item.qty : 0;
