const InfoPanel={
    ...
    template:`
        ...
        <div class="bg-white rounded-1 shadow p-4" id="check" style="scroll-margin-top: 6rem">
            ...
            <template v-if="order.travelDate">
                ...
                <template v-if="!tour.partner && !order.flash_sale">
                <div class="d-flex justify-content-end py-2 text-end border-dashed-bottom" v-if="showPromocode==false">
                    <div class="c-950 pointer tdu" v-on:click="showPromocode=true">Have a promo code?</div>
                </div>
                <div class="py-3 border-dashed-bottom" v-if="showPromocode==true">
                ...
                <template v-if="order.step==1">
                    <div class="pt-3 d-flex fw-600 c-950 align-items-end">
                        <div class="flex-grow-1">
                            <div>Total</div>
                        </div>
                        <div>
                         <div class="tdo c-950 text-right fz-09" v-if="oldPrice"><format-number v-bind:number="oldPrice" /></div>
                         <div class="c-blue"><format-number v-bind:number="order.totalPrice" /></div>
                        </div>
                    </div>

                </template>
                <template v-else>
                    <div class="pt-3 d-flex fw-600 c-950">
                        <div class="flex-grow-1">
                            <div>Total</div>
                        </div>
                        <div>
                ...
                <div class="btn btn-orange w-100 mt-3" v-on:click="order.step=3" v-else-if="order.step==2">
                    ...
                    <div class="d-xl-block d-none text-center c-950 mt-3 fz-09">
                        
                        <span class="d-inline-flex align-items-center"><i class="check-icon-green me-2"></i> Free cancellation </span>
                        <span class="d-inline-flex align-items-center ms-4"><i class="check-icon-green me-2"></i> Full refund if bad weather</span>
                    </div>
            ...
            <template v-else>
                <div class="py-3 d-flex border-dashed-bottom align-items-center">
                    ...
                    <div class="fz-12 fw-600">
                        <format-number v-bind:number="tour.show_price ? tour.show_price: tour.packages.pricelist[0].price" />
                    </div>
                </div>
        ...
        <Teleport to="body">
            <div class="d-xl-none d-block position-sticky bottom z-3 bg-white shadow" v-if="order.step!=3">
                <div class="container-xl py-3">
                    <div class="row align-items-center">
                        <template v-if="order.travelDate">
                            <div class="col" data-src="#booking-details" data-fancybox>
                                <div class="tdo c-950" v-if="oldPrice">
                                    <format-number v-bind:number="oldPrice" />
                                </div>
                                <div class="fz-12 fw-600 c-blue lh-1">
                                    <format-number v-bind:number="order.totalPrice" />
                                </div>
                                <div class="d-flex align-items-center tdu">
                                    <div class="c-500"><span>
                                        <format-date v-bind:date="order.travelDate" />
                                        </span>&nbsp; (<span v-text="membersView"></span>)
                                    </div>
                                </div>
                            </div>
                            <div class="col-auto">
                ...
                <div class="text-center c-950 py-2 fz-09">
                    <span class="d-inline-flex align-items-center"><i class="check-icon-green me-2"></i> Free cancellation </span>
                    <span class="d-inline-flex align-items-center ms-4"><i class="check-icon-green me-2"></i> Full refund if bad weather</span>
                </div>
            </div>
            <template>
                <div class="bottom-popup bg-white popup-medium p-xl-1 p-4 order-lg-0 order-1 rounded-1" id="booking-details">
                    ...
                    <div class="py-3 border-dashed-bottom c-500">
                        ...
                        <div class="mt-2 d-flex align-items-center" v-if="selectedRestaurant">
            ...
            <template>
                <div class="bottom-popup bg-white popup-large p-xl-1 p-4 rounded-1" id="more">
                    ...
                    <div class="border-dashed-bottom py-3" v-if="selectedRestaurant">
                        <div class="d-flex align-items-center">
    ...
    methods:{
        ...
        async checkPromo() {
            ...
            try {
                const response = await axios.get('/api/checkpromo/'+this.tour.id+'/'+trimmedCode);
    ...
    computed:{
        membersView(){
            if(this.order.allMembers>1){
               return  this.order.allMembers + ' persons'
            }else{
                return this.order.allMembers + ' person'
            }
        },


        fee1(){
            return this.order.totalPrice*0.12
        },
        fee2(){
            return 75000+10000*this.order.members
        },
        fee3(){
            return 100000+25000*this.order.members
        },
        feeAll(){
            return this.fee1 + this.fee2 + this.fee3
        },
        totalWithoutFee(){
            return this.order.totalPrice - this.feeAll
        },
        oldPrice(){
            if(order.step!=1 || !tour.percent) return false;
            if(tour.gross_price && this.tour.types_id==1){
                
                return this.tour.gross_price*this.order.allMembers
            }else{
                return (this.order.boatPrice+this.order.tourPrice)*(this.tour.percent+100)/100
            }
        },
        ...
        selectedRestaurant(){
            ...
            return  this.selectedProgram.restaurant?.find(r => r.id === this.order.selectedRestaurantId);
        },
        selectedTransfer(){
